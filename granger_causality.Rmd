...
For a stochastic process $X_t$ denote the entire set of past observations as $\overline{X}$. Furthermore let the **U**niverse of our study at time $t$. Lastly let $U_t - Y_t$ signify all information *except for* some process $Y_t$.
...
\sigma^2(Y|\overline{U}) &< \sigma^2(X|\overline{U - X}) \\
...
The definition of Granger causality is just saying that if we can explain $X$ using information on both $X$ *and* $Y$ better than we could by using information we already have about $X$, then $Y$ Granger causes (G-causes) $X$. Feedback then simply refers to $Y$ G-causing $X$ while $X$ G-causes $Y$. 

## Practical implementation

Testing whether some proposed stimulus $X_t$ Granger causes some response $Y_t$ is fairly simple. The current information $U_t$ is the period-$t$ observations $Y_t$ and $X_t$; the past observation -- $\overline{U}$ above -- are $\{ Y_{t-1} , Y_{t-2} , \dots,Y_{t-k} , X_{t-1} , X_{t-2} , \dots , X_{t-k} \}$. Testing condition for $X_t$ Granger-causing $Y_t$ can then be done by comparing two linear regression models:
